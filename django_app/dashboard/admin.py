...
class SystemConfigAdmin(admin.ModelAdmin):
    """Настройка источника данных: база PostgreSQL или файлы DBF.

    Здесь администратор переключает режим составления отчётов и графиков
    (вкладка «Отчёты»), а также задаёт путь к папке с файлами *.dbf.
    """

    list_display = ('data_source', 'dbf_dir', 'updated_at')
    fieldsets = (
        ('Источник данных', {
            'fields': ('data_source',),
            'description': 'Выберите, откуда брать данные для отчётов и графиков '
                           '(вкладка «Отчёты»): из базы данных PostgreSQL или из '
                           'архивных файлов DBF.',
        }),
        ('Файлы DBF', {
        ...
        ('Оформление', {
            'fields': ('logo',),
            'description': 'Изображение в шапке рядом с надписью «Молвест.Учет.Продукции» '
                           'вместо иконки-коробки (PNG с прозрачностью, высота ~30–40 px).',
        }),
    )
